...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        factors_list = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminationVariable in eliminationOrder:
            factors_list, factor_to_eliminate = joinFactorsByVariable(factors_list, eliminationVariable)
            if len(factor_to_eliminate.unconditionedVariables()) > 1:
                factors_list.append(eliminate(factor_to_eliminate, eliminationVariable))
        final_factor = joinFactors(factors_list)
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            distribution[particle] += self.getObservationProb(observation, pacman_pos, particle, jail_pos)
