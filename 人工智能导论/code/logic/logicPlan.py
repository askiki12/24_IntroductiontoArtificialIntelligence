...
def sentence1() -> Expr:
    ...
    lis = []
    lis.append(A | B)
    lis.append(notA % notB_or_C)
    lis.append(disjoin([notA, notB, C]))
    return conjoin(lis)
...
def sentence3() -> Expr:
    ...
    lis = []
    word1 = PacmanAlive_1 % ((PacmanAlive_0 & (~PacmanKilled_0)) |
                             ((~PacmanAlive_0) & PacmanBorn_0))
    word2 = ~(PacmanAlive_0 & PacmanBorn_0)
    word3 = PacmanBorn_0
    lis.append(word1)
    lis.append(word2)
    lis.append(word3)
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # return PropSymbolExpr(pacman_str, x, y, time=now) % (disjoin(possible_causes) & ~PropSymbolExpr(wall_str, x, y))
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(move, time=t) for move in DIRECTIONS]))
    if sensorModel != None:
